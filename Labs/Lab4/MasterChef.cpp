...
// Creates and starts a timer given a pointer to the step to start and when it
...
// Mark the step to be removed as a dependency, and trigger the remove dep
// handler. TO COMPLETE Section 2
static void timerHandler(int sig, siginfo_t *si, void *uc) {
  ...
  if (comp_item) {
    // Print completion (required by spec)
...
void RemoveDepHandler(int sig) {
  /* TODO This Section - 3 */
  // For each completed step, remove it as a dependency from all remaining
  // steps. (Idempotent: removing an already-removed dep is harmless.)
...
// Continuously check what steps are ready to be run, and start timers for them
// with makeTimer() run until all steps are done. To Complete - Section 1
int main(int argc, char **argv) {
  ...
  // Associate the appropriate handler with the SIGUSR1 signal, for removing
  // dependencies
  ...
  // Until all steps have been completed, check if steps are ready to be run and
  // create a timer for them if so
  const int totalSteps = recipeSteps->Count();
  while ((int)completedSteps->size() < totalSteps) {
    // Find all ready steps (dependencies empty and not already running)
    vector<Step *> ready = recipeSteps->GetReadySteps();
    for (Step *s : ready) {
      // Start a one-shot timer for each ready step
      s->running = true;
      makeTimer(s, s->duration);
    }
    // Sleep a bit to avoid busy-waiting; timers/handlers will drive progress
    usleep(10000); // 10ms
  }
  /* End Section - 1 */
